...
func NewAdminHandlers(admin *services.AdminService, vendors *repo.VendorsRepo, payouts *repo.PayoutsRepo, disputes *repo.DisputesRepo) *AdminHandlers {
	return &AdminHandlers{admin: admin, vendors: vendors, payouts: payouts, disputes: disputes}
}
...
func (h *AdminHandlers) RejectPayout(w http.ResponseWriter, r *http.Request) {
	...
	if req.Reason == "" { req.Reason = "rejected" }
