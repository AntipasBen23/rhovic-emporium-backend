...
func NewCheckoutService(pool *pgxpool.Pool, orders *repo.OrdersRepo, payments *repo.PaymentsRepo, checkout *repo.CheckoutRepo, settings *repo.SettingsRepo, ps *paystack.Client) *CheckoutService {
	return &CheckoutService{pool: pool, orders: orders, payments: payments, checkout: checkout, settings: settings, paystack: ps}
}
...
func (s *CheckoutService) Checkout(ctx context.Context, buyerID string, req CheckoutRequest) (CheckoutResponse, error) {
	...
	err := db.WithTx(ctx, s.pool, func(tx pgx.Tx) error {
		...
		// init paystack (outside tx would be ideal, but we need payment record consistency)
	...
	// re-init to get auth_url in dev vs real; simplest: call again (Paystack returns same ref)
	initRes, _ := s.paystack.Initialize(ctx, paystack.InitRequest{
